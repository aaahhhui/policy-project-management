...
- 本地验收基线：后端 380 passed / 2 skipped，Ruff 通过，mypy 81 个源文件通过；前端 29 个文件 / 131 项测试通过，Vue 类型检查和生产构建通过。
- 一次性 SQLite 浏览器回退验证通过桌面端和 390x844 下的负责人、对接人、只读用户权限检查，以及 `0006 -> 0005 -> 0006` 迁移往返。
