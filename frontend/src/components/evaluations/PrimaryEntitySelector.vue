...
<template>
  <form class="primary-selector" @submit.prevent="submit">
    ...
    <button type="submit">{{ current ? "更新主申报企业" : "确定主申报企业" }}</button>
  </form>
</template>

<style scoped>
.primary-selector{margin-top:1rem;padding:1rem;border:1px solid #d6e1ec;background:#fff}.primary-selector>p:first-child{margin:0;color:#74899e;font-size:.72rem}.primary-selector h3{margin:.25rem 0 .8rem;font-family:"Noto Serif SC","Songti SC",serif}.candidate{display:flex;gap:.55rem;padding:.55rem;border-top:1px solid #e3ebf3}.reason{display:grid;gap:.3rem;margin-top:.8rem;color:#60758d;font-size:.76rem}textarea{padding:.55rem;border:1px solid #b9c9d8;font:inherit}.primary-selector p[role=alert]{color:#9b1c1c}.primary-selector button{margin-top:.8rem;padding:.55rem .85rem;border:1px solid #174f7e;background:#174f7e;color:#fff;font-weight:700}
</style>
