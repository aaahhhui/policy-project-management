...
const primaryEntitySeedCode = ref("");
const reason = ref("");
const error = ref("");
...
async function submit() {
  ...
  if (conclusion.value === "recommend_apply" && !primaryEntitySeedCode.value) {
    error.value = "请选择主申报企业";
    return;
  }
  if (!conclusion.value || !props.evaluation.summary) return;
  saving.value = true;
  try {
    await confirmEvaluation(props.evaluation.id, {
      ...
      primary_entity_seed_code: primaryEntitySeedCode.value || null,
    });
    emit("confirmed");
  } catch {
...
<template>
  <form class="confirmation-form" @submit.prevent="submit">
    ...
    <fieldset class="conclusion-options">
      <legend>政策处理结论</legend>
      <label v-for="option in conclusionOptions" :key="option.value">
        <input v-model="conclusion" type="radio" name="policy-conclusion" :value="option.value" />
        <span>{{ option.label }}</span>
      </label>
    </fieldset>
    <fieldset v-if="conclusion === 'recommend_apply'" class="primary-options">
      <legend>主申报企业</legend>
      <label v-for="entity in entities" :key="entity.entity_seed_code">
...
.conclusion-options,.primary-options{display:flex;flex-wrap:wrap;gap:.7rem 1rem;margin:1rem 0;padding:.8rem;border:1px solid #d6e1ec}.conclusion-options legend,.primary-options legend{color:#60758d;font-size:.76rem;font-weight:800}.conclusion-options label,.primary-options label{display:flex;align-items:center;gap:.35rem}.conclusion-options input,.primary-options input{padding:0}
</style>
