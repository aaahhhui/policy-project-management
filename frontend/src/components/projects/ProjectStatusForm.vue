...
const saving = ref(false); const error = ref(""); const conflict = ref(false);
...
async function submit(): Promise<void> {
  ...
  catch (caught) { conflict.value = isVersionConflict(caught); error.value = businessErrorMessage(caught, "状态变更未完成，请稍后重试。"); }
...
<template>
  <form v-if="options.length" class="project-status-form" @submit.prevent="submit">
    ...
    <p v-if="error" role="alert">{{ error }} <button v-if="conflict" type="button" data-reload-project @click="emit('reload')">重新加载</button></p><button type="submit" :disabled="saving">{{ saving ? "正在提交……" : "变更状态" }}</button>
  </form>
</template>
