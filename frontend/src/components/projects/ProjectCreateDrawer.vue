...
const props = defineProps<{ open: boolean; keyGenerator?: () => string }>();
...
async function loadPolicies(page = 1): Promise<void> {
  ...
  try {
    const result = await getConvertiblePolicies(page, 20);
    policies.value = result.items; policyPage.value = result.page; policyTotal.value = result.total;
...
<template>
  <el-drawer v-if="isOwner" :model-value="open" :append-to-body="true" size="min(42rem, 100%)" @close="close">
    ...
    <form class="project-create-form" @submit.prevent="submit">
      ...
      <template v-else>
        <label>可转换政策<select v-model.number="draft.policy_id" aria-label="可转换政策" required @change="onPolicyChange"><option v-for="policy in policies" :key="policy.id" :value="policy.id">{{ policy.title }}</option></select></label>
        ...
        <nav v-if="policyTotal > 20" class="policy-pages" aria-label="可转换政策分页"><button type="button" :disabled="policyPage === 1" @click="loadPolicies(policyPage - 1)">上一页</button><span>第 {{ policyPage }} 页</span><button type="button" :disabled="policyPage * 20 >= policyTotal" @click="loadPolicies(policyPage + 1)">下一页</button></nav>
      </template>
    </form>
  </el-drawer>
</template>
