...
const statusLabels: Record<string, string> = {
  pending_application: "待申报", submitted: "已提交", succeeded: "已成功", rejected: "未获批", terminated: "已终止",
};
const conclusionLabels: Record<string, string> = {
  recommend_apply: "建议申报", watch: "持续关注", not_recommend_apply: "暂不建议申报", unable_to_determine: "无法判断",
};
...
<template>
  <section class="project-detail" aria-labelledby="project-detail-title">
    ...
    <template v-else-if="project">
      <header class="heading"><p class="eyebrow">项目管理 · 可追溯台账</p><h1 id="project-detail-title">{{ project.name }}</h1><p class="status">{{ statusLabels[project.status] }}</p></header>
      ...
      <section v-if="canShowMutations" data-project-mutations class="mutations" aria-label="项目维护操作">
        <h2>项目维护</h2>
        <ProjectEditForm v-if="project.capabilities.can_edit_project || project.capabilities.can_update_progress" :project="project" @updated="replaceProject" @reload="load" />
        <ProjectStatusForm v-if="project.capabilities.can_transition" :project="project" @updated="replaceProject" />
        <ProjectCorrectionDialog v-if="project.capabilities.can_correct_status" :project="project" mode="status" @updated="replaceProject" />
        <ProjectCorrectionDialog v-if="project.capabilities.can_correct_primary_entity" :project="project" mode="primary-entity" @updated="replaceProject" />
      </section>
